...
const TabBar = ({ active, onNav, onVoice }) => {
    ...
    <div style={{
      ...
      <div className="glass" style={{
        ...
        {tabs.map(t => {
          ...
          return (
            <div key={t.id} onClick={() => t.id === 'more' ? setMoreOpen(v => !v) : (setMoreOpen(false), onNav(t.id))} className="tap" style={{
...
const Toast = ({ notif, onDismiss }) => {
  ...
  const icons = { gps: 'location', family: 'family', habit: 'target', system: 'sparkle' };
  const Ic = I[icons[notif.kind]];
  return (
...
Object.assign(window, { TabBar, VoiceOverlay, Toast, Screens });
