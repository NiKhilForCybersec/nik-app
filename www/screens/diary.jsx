...
const DiaryScreen = ({ onNav, themeId = 'solo-leveling' }) => {
  const [entries, setEntries] = React.useState(MOCK_DIARY);
  const [composeOpen, setComposeOpen] = React.useState(false);
  const [activeEntry, setActiveEntry] = React.useState(null);
  const [filter, setFilter] = React.useState('all'); // all | photos | voice | mood
  ...
  const filtered = entries.filter(e => {
    ...
    if (filter === 'mood') return e.mood >= 4;
    return true;
  });
  ...
  return (
    <div style={{ padding: '8px 16px 80px' }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div className="display" style={{ fontSize: 26, fontWeight: 'var(--display-weight, 500)', lineHeight: 1.1, marginTop: 4, textTransform: 'var(--display-case, none)', letterSpacing: 'var(--display-tracking, 0)' }}>Your private record</div>
      </div>
      ...
      <div onClick={() => setComposeOpen(true)} className="tap" style={{ position: 'fixed', right: 'calc(50% - 158px)', bottom: 100, zIndex: 30, width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg, oklch(0.78 0.16 var(--hue)), oklch(0.55 0.22 calc(var(--hue) + 60)))', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 30px oklch(0.78 0.16 var(--hue) / 0.5)' }}>
        <I.plus size={22} stroke="#06060a" sw={2.4}/>
      </div>

      {composeOpen && <DiaryComposeSheet onClose={() => setComposeOpen(false)} onAdd={addEntry} prompt={composeOpen?.prompt}/>}
...
window.MOCK_DIARY = MOCK_DIARY;
window.ON_THIS_DAY = ON_THIS_DAY;
