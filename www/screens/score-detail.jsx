...
const ScoreDetailScreen = ({ onNav, themeId }) => {
  const s = window.MOCK_SCORE;
  const [activePillar, setActivePillar] = React.useState(null);
  const t = window.getTheme ? window.getTheme(themeId) : null;
  const rankPrefix = t?.vocab?.rankPrefix || 'OPERATIVE';
  ...
  return (
    <div style={{ padding: '8px 16px 80px' }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div className="display" style={{ fontSize: 26, fontWeight: 'var(--display-weight, 500)', lineHeight: 1.1, marginTop: 4, textTransform: 'var(--display-case, none)', letterSpacing: 'var(--display-tracking, 0)' }}>Your standing</div>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, position: 'relative' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)', marginTop: 4, display: 'flex', alignItems: 'center', gap: 5 }}>
              <span style={{ color: s.delta7d > 0 ? 'oklch(0.75 0.18 140)' : 'oklch(0.75 0.18 25)' }}>{s.delta7d > 0 ? '↗' : '↘'} {Math.abs(s.delta7d)}</span> 7-day · +{s.todayContribution} today
            </div>
          </div>
          <div style={{ width: 80, height: 80, position: 'relative', flexShrink: 0 }}>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ fontSize: 8, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)', marginTop: 1 }}>{nextRankAt - s.total} TO NEXT</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div key={p.id} onClick={() => setActivePillar(activePillar === p.id ? null : p.id)} className="tap" style={{ padding: '10px 0', borderBottom: '1px solid var(--hairline)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                <div style={{ width: 28, height: 28, borderRadius: 8, background: `oklch(0.78 0.16 ${p.color} / 0.18)`, border: `1px solid oklch(0.78 0.16 ${p.color} / 0.3)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {I[p.icon] && React.createElement(I[p.icon], { size: 13, stroke: `oklch(0.85 0.16 ${p.color})` })}
                </div>
                <div style={{ flex: 1 }}>
      ...
      <div className="glass fade-up" style={{ padding: 14, marginBottom: 10 }}>
        ...
        {s.backlog.map(b => {
          const pillar = window.SCORE_PILLARS.find(p => p.id === b.pillar);
          return (
            <div key={b.id} className="fade-up" style={{ padding: 10, borderRadius: 10, background: 'oklch(1 0 0 / 0.03)', border: '1px solid var(--hairline)', marginBottom: 6 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                <div style={{ width: 22, height: 22, borderRadius: 6, background: `oklch(0.78 0.16 ${pillar.color} / 0.18)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                  {I[pillar.icon] && React.createElement(I[pillar.icon], { size: 10, stroke: `oklch(0.85 0.16 ${pillar.color})` })}
                </div>
      ...
      <div className="glass fade-up" style={{ padding: 14 }}>
        ...
        {s.recent.map((r, i) => {
          const pillar = window.SCORE_PILLARS.find(p => p.id === r.pillar);
...
window.ScoreDetailScreen = ScoreDetailScreen;
