...
type OnboardData = {
  ...
  partner: boolean;
  chronotype: string;
  perms: { health: boolean; calendar: boolean; location: boolean; contacts: boolean };
  voice: string;
  theme: string;
};

export default function OnboardScreen({ onNav }: ScreenProps) {
  const onDone = () => onNav('home');
  ...
  const set = (patch: Partial<OnboardData>) => setData(d => ({ ...d, ...patch }));
        ...
        {cur === 'family' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
              {([['solo','Just me'],['parent','Me + kids'],['couple','Me + partner'],['family','Whole family']] as const).map(([k, l]) => (
