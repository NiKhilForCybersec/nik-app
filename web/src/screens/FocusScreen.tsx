...
type Task = {
  id: string;
  title: string;
  energy: 'deep' | 'medium' | 'shallow';
  est: number;
  pillar: string;
  icon: string;
};

type DistractionReason = { id: string; label: string; icon: string };
...
const DISTRACTION_REASONS: DistractionReason[] = [
...
const ENERGY_PRESETS: Record<string, { duration: number; break: number; label: string; desc: string }> = {
...
export default function FocusScreen({ onExit }: ScreenProps) {
  const [stage, setStage] = React.useState<'setup' | 'running' | 'paused' | 'report'>('setup');
  const [task, setTask] = React.useState<Task>(FOCUS_TASKS[0]);
  const [duration, setDuration] = React.useState(50);
  const [preset, setPreset] = React.useState('deep');
  const [strictness, setStrictness] = React.useState<'soft' | 'medium' | 'hard'>('medium');
  const [elapsed, setElapsed] = React.useState(0);
  const [distractions, setDistractions] = React.useState<Distraction[]>([]);
  const [showLeave, setShowLeave] = React.useState(false);
  const [pendingDistraction, setPendingDistraction] = React.useState<{ at: number } | null>(null);
  ...
  const askAi = () => {
    setAiThinking(true);
    setTimeout(() => {
      const energySuggestions: Record<string, { d: number; p: string; reason: string }> = {
  ...
  const logDistraction = (reason: DistractionReason) => {
...
type SetupProps = {
  ...
  setStrictness: (s: 'soft' | 'medium' | 'hard') => void;
  askAi: () => void;
  aiThinking: boolean;
  startSession: () => void;
  onExit?: () => void;
...
const FocusSetup: React.FC<SetupProps> = ({ task, setTask, duration, setDuration, preset, setPreset, strictness, setStrictness, askAi, aiThinking, startSession }) => (
  <div style={{ padding: '8px 16px 100px' }}>
    <div style={{ marginBottom: 14 }}>
      ...
      <div className="display" style={{ fontSize: 26, fontWeight: 'var(--display-weight, 500)' as any, lineHeight: 1.1, marginTop: 4, textTransform: 'var(--display-case, none)' as any, letterSpacing: 'var(--display-tracking, 0)' as any }}>What are we doing?</div>
    </div>
...
// ── Tree ──────────────────────────────────────────────
const FocusTree: React.FC<{ pct: number; distractions: number }> = ({ pct, distractions }) => {
...
const FocusReport: React.FC<ReportProps> = ({ task, duration, distractions, onExit, restart }) => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, padding: '50px 16px 30px', background: 'radial-gradient(ellipse at top, oklch(0.18 0.04 var(--hue)) 0%, oklch(0.06 0.02 var(--hue)) 70%)', overflowY: 'auto', zIndex: 100 }}>
      ...
      <div className="display" style={{ fontSize: 28, fontWeight: 'var(--display-weight, 500)' as any, lineHeight: 1.1, marginBottom: 4 }}>{focusScore >= 90 ? 'Outstanding.' : focusScore >= 70 ? 'Good work.' : 'Done. Keep going.'}</div>
