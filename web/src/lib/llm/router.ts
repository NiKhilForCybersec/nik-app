 ...
 *
 * Today: Anthropic primary, OpenAI fallback. The future on-device
 * SLM (Phi-3 / Gemma 3 / Llama 3.2 / Qwen 2.5) drops in with no other
 * code changes — register it in the providers list, set its
 * `handles: ['trivial']`, and the router prefers it when complexity
 * is trivial + it's available.
 *
 * The MCP layer is the abstraction the model interacts with. Models
 * are interchangeable; the registry isn't.
 */
