/* Nik — Meds add/AI/detail sheets
   Ported from www/screens/meds-sheets.jsx */
...
// Schedule label helper (mirrors MedsScreen)
const scheduleLabel = (s: Schedule): string => {
  ...
  switch (s.kind) {
    ...
    case 'course': return `Course · day ${s.courseDay || 1}/${s.day || s.courseOf || 7} · ${s.perDay}× daily`;
...
// ── Local helpers ──────────────────────────────────
const inputStyle = (): React.CSSProperties => ({ width: '100%', padding: 10, borderRadius: 10, background: 'oklch(1 0 0 / 0.06)', border: '1px solid var(--hairline)', color: 'var(--fg)', fontSize: 13, fontFamily: 'var(--font-body, Inter)', outline: 'none', boxSizing: 'border-box' });

const SectionLabel: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div style={{ fontSize: 9, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)', letterSpacing: 1.5, marginBottom: 5 }}>{children}</div>
);

const Pill: React.FC<{ active: boolean; onClick: () => void; children: React.ReactNode }> = ({ active, onClick, children }) => (
  <div onClick={onClick} className="tap" style={{ padding: '6px 12px', borderRadius: 99, fontSize: 11, background: active ? 'oklch(0.78 0.16 var(--hue) / 0.2)' : 'oklch(1 0 0 / 0.04)', border: '1px solid ' + (active ? 'oklch(0.78 0.16 var(--hue) / 0.4)' : 'var(--hairline)'), color: active ? 'oklch(0.9 0.14 var(--hue))' : 'var(--fg-2)' }}>{children}</div>
);

const NumField: React.FC<{ label: string; value: number | string; onChange: (v: any) => void; min?: number; max?: number }> = ({ label, value, onChange, min = 1, max = 30 }) => (
  <div>
...
// ── AddMedSheet ────────────────────────────────────
type AddMedSheetProps = {
  onClose: () => void;
  onAdd: (med: Med) => void;
};

export const AddMedSheet: React.FC<AddMedSheetProps> = ({ onClose, onAdd }) => {
  ...
  const [onDays, setOnDays] = useState<number | string>(21);
  const [offDays, setOffDays] = useState<number | string>(7);
  const [courseDays, setCourseDays] = useState<number | string>(7);
  const [perDay, setPerDay] = useState<number | string>(3);
  const [maxPerDay, setMaxPerDay] = useState<number | string>(3);
  const [minGap, setMinGap] = useState<number | string>(6);
  const [forWho, setForWho] = useState('self');
  const [note, setNote] = useState('');

  const DAYS: Array<[string, string]> = [['mon','M'],['tue','T'],['wed','W'],['thu','T'],['fri','F'],['sat','S'],['sun','S']];
  ...
  const buildSchedule = (): Schedule => {
    switch (kind) {
      ...
      case 'course': return { kind, days: undefined, perDay: +perDay, courseDay: 1, courseOf: +courseDays, times: ['08:00','14:00','20:00'].slice(0, +perDay) };
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'oklch(0 0 0 / 0.7)', backdropFilter: 'blur(12px)', zIndex: 80, display: 'flex', alignItems: 'flex-end' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="fade-up" style={{ width: '100%', background: 'var(--theme-bg2, oklch(0.14 0.02 260))', border: '1px solid var(--hairline)', borderRadius: '24px 24px 0 0', padding: 18, paddingBottom: 36, maxHeight: '92%', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 5, marginBottom: 10, flexWrap: 'wrap' }}>
          {([['self', 'Me'], ['kiaan', 'Kiaan'], ['anya', 'Anya']] as const).map(([k, l]) => (
...
const parseAi = (text: string): Med => {
  const t = text.toLowerCase();
  const med: Med = { name: '', dose: '1 dose', source: 'voice', icon: 'pill', for: 'self', note: 'Parsed by AI from: "' + text + '"', schedule: { kind: 'daily', time: '08:00' } };
  // name
  ...
  if (everyN) med.schedule = { kind: 'every-n-days', n: +everyN[1], time };
    ...
    const n = +(t.match(/(\d+) (times|x)/) as RegExpMatchArray)[1];
    const courseMatch = t.match(/for (\d+) days?/);
    if (courseMatch) med.schedule = { kind: 'course', perDay: n, courseDay: 1, courseOf: +courseMatch[1], times: ['08:00','14:00','20:00'].slice(0, n) };
...
type AiMedChatSheetProps = {
  onClose: () => void;
  onAdd: (med: Med) => void;
};

export const AiMedChatSheet: React.FC<AiMedChatSheetProps> = ({ onClose, onAdd }) => {
  const [text, setText] = useState('');
  const [parsed, setParsed] = useState<Med | null>(null);
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'oklch(0 0 0 / 0.7)', backdropFilter: 'blur(12px)', zIndex: 80, display: 'flex', alignItems: 'flex-end' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="fade-up" style={{ width: '100%', background: 'var(--theme-bg2, oklch(0.14 0.02 260))', border: '1px solid var(--hairline)', borderRadius: '24px 24px 0 0', padding: 18, paddingBottom: 36, maxHeight: '88%', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          ...
          <div>
            <div className="display" style={{ fontSize: 16, fontWeight: 600 }}>Tell Nik</div>
        ...
        {parsed && (
          <div className="fade-up">
            <SectionLabel>NIK UNDERSTOOD</SectionLabel>
...
// ── MedDetailSheet ─────────────────────────────────
type MedDetailSheetProps = {
  med: Med;
  onClose: () => void;
  onRemove: () => void;
};

export const MedDetailSheet: React.FC<MedDetailSheetProps> = ({ med, onClose, onRemove }) => (
  <div style={{ position: 'absolute', inset: 0, background: 'oklch(0 0 0 / 0.7)', backdropFilter: 'blur(12px)', zIndex: 80, display: 'flex', alignItems: 'flex-end' }} onClick={onClose}>
