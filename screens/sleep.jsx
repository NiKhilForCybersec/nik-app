...
const SleepScreen = ({ onNav }) => {
  const [tab, setTab] = React.useState('overview'); // overview | wind-down | dreams | alarm
  ...
  return (
    <div style={{ padding: '8px 16px 100px' }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <div className="display" style={{ fontSize: 28, fontWeight: 'var(--display-weight, 500)', lineHeight: 1.1, marginTop: 4, textTransform: 'var(--display-case)', letterSpacing: 'var(--display-tracking)' }}>Sleep</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 12, padding: 3, background: 'var(--input-bg)', borderRadius: 12 }}>
        {[['overview','Stages'],['wind-down','Wind-down'],['dreams','Dreams'],['alarm','Alarm']].map(([k,l]) => (
        ...
        <>
          ...
          <div className="glass fade-up" style={{ padding: 14, marginBottom: 10 }}>
            ...
            <div style={{ position: 'relative', height: 80 }}>
              {['awake','rem','light','deep'].map((stage, row) => {
                const y = row * 20;
                const colors = { awake: 'oklch(0.7 0.18 30)', rem: 'oklch(0.7 0.18 var(--hue))', light: 'oklch(0.6 0.12 var(--hue))', deep: 'oklch(0.45 0.15 var(--hue))' };
                return (
        ...
        <>
          ...
          <div className="glass fade-up" style={{ padding: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              {['Rain on a tin roof', 'Brown noise', 'Forest at dusk', 'Tibetan bowls'].map(s => (
                <div key={s} className="tap" style={{ padding: 10, borderRadius: 10, background: 'var(--input-bg)', fontSize: 11, color: 'var(--fg-2)' }}>{s}</div>
              ))}
            </div>
          </div>
        </>
        ...
        <>
          ...
          <div className="glass fade-up" style={{ padding: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[['Soft chimes — fade in 3 min', true], ['Birdsong + light increase', false], ['Vibration only', false]].map(([l, on], i) => (
