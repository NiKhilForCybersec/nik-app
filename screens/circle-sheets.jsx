/* Nik — Family Circle: detail sheets
   - MemberDetailSheet: full profile with privacy gating
   - PrivacySheet: per-person matrix with tier presets
   - ViewLogSheet: who viewed what about you
*/

// ── Sheet wrapper ────────────────────────────────────────
const CircleSheet = ({ title, sub, onClose, children, accent }) => {
  return (
...
const LockedSection = ({ label, owner, viewer, onRequest }) => (
...
// ── Member detail sheet ──────────────────────────────────
const MemberDetailSheet = ({ member: m, viewerId, sharing, onClose, onNav }) => {
  const isMe = m.id === viewerId;
  const can = (cat) => canCircleView(viewerId, m.id, cat, sharing);
      ...
      {can('mood') ? (
        <div className="glass" style={{ padding: 14, marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 4, height: 32, alignItems: 'flex-end' }}>
            {m.health.mood.trend7.map((mood, i) => {
...
const SectionLabel = ({ children }) => (
  <div style={{ fontSize: 10, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)', letterSpacing: 1.5, marginBottom: 8, marginTop: 4 }}>{children.toString().toUpperCase()}</div>
);
...
const PrivacySheet = ({ me, members, sharing, onChange, onClose }) => {
  const others = members.filter(m => m.id !== me);
  // Local mutable copy of sharing
  const [local, setLocal] = React.useState(() => JSON.parse(JSON.stringify(sharing)));
  const [activeViewer, setActiveViewer] = React.useState(others[0]?.id);
  ...
  const activeCats = isCustom ? currentTier.cats : (TRUST_TIERS[currentTier]?.cats || []);
...
// ── View log sheet ────────────────────────────────────────
const ViewLogSheet = ({ me, members, log, onClose }) => {
  const [tab, setTab] = React.useState('viewed-me'); // viewed-me | i-viewed
...
Object.assign(window, { MemberDetailSheet, PrivacySheet, ViewLogSheet, CircleSheet, SectionLabel, StatCard, LockedSection });
