/* Aether — Diary compose + detail sheets */
...
const DiaryComposeSheet = ({ onClose, onAdd, prompt }) => {
  ...
  const [letterToFuture, setLetterToFuture] = React.useState(false);
  const [futureDate, setFutureDate] = React.useState('1y');
  const [tagInput, setTagInput] = React.useState('');
  const [aiAssist, setAiAssist] = React.useState(false);
  ...
  const acceptAi = () => {
    ...
    setTags([...new Set([...tags, ...aiSuggestion.tags])]);
      ...
      <div onClick={e => e.stopPropagation()} className="fade-up" style={{ width: '100%', background: 'var(--sheet-bg)', border: '1px solid var(--hairline)', borderRadius: '24px 24px 0 0', padding: 18, paddingBottom: 28, maxHeight: '94%', overflowY: 'auto' }}>
        ...
        {!aiSuggestion && (
          <div onClick={aiHelp} className="tap" style={{ padding: 10, borderRadius: 10, marginBottom: 10, background: 'oklch(0.78 0.16 var(--hue) / 0.08)', border: '1px dashed oklch(0.78 0.16 var(--hue) / 0.3)', display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--fg-2)' }}>
            ...
            {aiAssist ? <span style={{ color: 'oklch(0.85 0.14 var(--hue))' }}>Aether is thinking…</span> : 'Let Aether help expand this'}
          </div>
        )}
        {aiSuggestion && (
          <div className="glass" style={{ padding: 12, marginBottom: 10, borderColor: 'oklch(0.78 0.16 var(--hue) / 0.3)' }}>
            <div style={{ fontSize: 9, color: 'oklch(0.85 0.14 var(--hue))', fontFamily: 'var(--font-mono)', letterSpacing: 1, marginBottom: 6 }}>AETHER SUGGESTION</div>
...
const DiaryEntrySheet = ({ entry, onClose }) => (
  <div style={{ position: 'absolute', inset: 0, background: 'var(--scrim)', backdropFilter: 'blur(16px)', zIndex: 80, overflowY: 'auto' }} onClick={onClose}>
    <div onClick={e => e.stopPropagation()} className="fade-up" style={{ minHeight: '100%', padding: '60px 20px 40px', maxWidth: '100%' }}>
      ...
      {entry.title && <div className="display" style={{ fontSize: 26, fontWeight: 'var(--display-weight, 500)', lineHeight: 1.2, marginBottom: 12, color: 'var(--fg)' }}>{entry.title}</div>}
      ...
      {entry.aiPrompt && (
        <div style={{ marginTop: 18, padding: 12, borderRadius: 10, background: 'oklch(0.78 0.16 var(--hue) / 0.08)', borderLeft: '2px solid oklch(0.78 0.16 var(--hue))' }}>
          <div style={{ fontSize: 9, color: 'oklch(0.85 0.14 var(--hue))', fontFamily: 'var(--font-mono)', letterSpacing: 1, marginBottom: 4 }}>AETHER NOTICED</div>
...
const diaryInputStyle = () => ({ width: '100%', padding: 11, borderRadius: 10, background: 'oklch(1 0 0 / 0.04)', border: '1px solid var(--hairline)', color: 'var(--fg)', fontSize: 13, outline: 'none', boxSizing: 'border-box' });

Object.assign(window, { DiaryComposeSheet, DiaryEntrySheet, MOOD_LABELS, MOOD_EMOJIS, AI_DIARY_PROMPTS });
