...
const scheduleLabel = (s) => {
  ...
  switch (s.kind) {
    ...
    case 'course': return `Course · day ${s.courseDay || 1}/${s.days} · ${s.perDay}× daily`;
...
const MedsScreen = ({ onNav, themeId = 'solo-leveling' }) => {
  const T = getTheme(themeId);
  const [meds, setMeds] = React.useState(MEDS_SEED);
  const [filter, setFilter] = React.useState('all'); // all | today | self | kids | course
  const [addOpen, setAddOpen] = React.useState(false);
  const [aiChatOpen, setAiChatOpen] = React.useState(false);
  const [selected, setSelected] = React.useState(null);

  const toggleTaken = (id) => setMeds(ms => ms.map(m => {
    if (m.id !== id) return m;
    if (m.schedule.kind === 'prn') return { ...m, takenToday: (m.takenToday || 0) + 1 };
    return { ...m, takenToday: !m.takenToday, streak: !m.takenToday ? (m.streak || 0) + 1 : Math.max(0, (m.streak || 0) - 1) };
  }));
  ...
  const filtered = meds.filter(m => {
    ...
    if (filter === 'today') return m.schedule.kind === 'daily' || m.schedule.kind === 'every-n-days' || m.schedule.kind === 'course' || (m.schedule.kind === 'cycled' && m.schedule.cycleDay <= m.schedule.onDays);
    ...
    <div style={{ padding: '8px 16px 80px' }}>
      ...
      <div style={{ marginBottom: 14, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <div>
          ...
          <div className="display" style={{ fontSize: 26, fontWeight: 'var(--display-weight, 500)', lineHeight: 1.1, marginTop: 4, textTransform: 'var(--display-case, none)', letterSpacing: 'var(--display-tracking, 0)' }}>Medications</div>
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10, overflowX: 'auto', paddingBottom: 2 }}>
        {[['all', 'All', meds.length], ['today', 'Today', dueCount + takenCount], ['self', 'Mine', meds.filter(m=>m.for==='self').length], ['kids', 'Kids', meds.filter(m=>m.for!=='self').length], ['course', 'Course', meds.filter(m=>['course','tapered'].includes(m.schedule.kind)).length]].map(([k, l, n]) => (
...
// ── MED CARD ───────────────────────────────────────
const MedCard = ({ med, onToggle, onOpen }) => {
  ...
  const kid = med.for !== 'self' && (window.KIDS?.[med.for]);
  return (
    <div className="glass fade-up" style={{ padding: 12, display: 'flex', alignItems: 'flex-start', gap: 10, position: 'relative', borderColor: due ? 'oklch(0.78 0.16 var(--hue) / 0.25)' : undefined }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }} onClick={onOpen}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', marginBottom: 2 }}>
          ...
          {kid && <Chip tone="accent" size="sm">{kid.name.toUpperCase()}</Chip>}
        ...
        <div style={{ display: 'flex', gap: 8, fontSize: 10, color: 'var(--fg-3)', fontFamily: 'var(--font-mono)', flexWrap: 'wrap' }}>
          ...
          {med.streak > 0 && <span>· 🔥 {med.streak}d</span>}
        </div>
      ...
      {isPRN ? (
        <div onClick={onToggle} className="tap" style={{ padding: '7px 10px', borderRadius: 10, background: 'oklch(1 0 0 / 0.05)', border: '1px solid var(--hairline)', fontSize: 11, color: 'var(--fg)', display: 'flex', alignItems: 'center', gap: 4 }}>
          <I.plus size={10}/> Log ({med.takenToday || 0}/{med.schedule.maxPerDay})
        </div>
      ) : (
...
const MedQuickAddCard = ({ icon, title, sub, onClick }) => {
